...
We also have other versions of compressed control data, where duplicates are not removed (indicated with `.dup`, and used with the `--dup` option) or the number of controls are subsampled (indicated with `.reduced`, and used with the `--reduced` option). Please see the **OtherControlData** folder. 
