...
You can also obtain files from [Google Drive](https://drive.google.com/open?id=1Xh6Fjah1LoRMmbaJA7_FzxYcbqmpNUPZ) or [our data repository](https://dada.cs.washington.edu/aicontrol/). <>We also have other versions of compressed control data, where duplicates are not removed (indicated with `.dup`, and used with the `--dup` option) or the number of controls are subsampled (indicated with `.reduced`, and used with the `--reduced` option).
