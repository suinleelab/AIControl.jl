...
We also have other versions where duplicates are not removed (indicated with `.dup`, and used with the `--dup` option) or control files are subsampled. Please see the **OtherControlData** folder. 
